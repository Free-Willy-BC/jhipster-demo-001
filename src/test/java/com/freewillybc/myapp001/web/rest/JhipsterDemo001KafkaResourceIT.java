...
import com.freewillybc.myapp001.config.EmbeddedKafka;
import com.freewillybc.myapp001.config.KafkaSseConsumer;
import com.freewillybc.myapp001.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class JhipsterDemo001KafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/jhipster-demo-001-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jhipster-demo-001-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/jhipster-demo-001-kafka/unregister"));
                return;
            }
        }
